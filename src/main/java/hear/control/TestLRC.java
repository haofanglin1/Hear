...
public class TestLRC {
    ...
    public static void main(String[] args) {
        String path = "F:\\cloudmusicdown\\林俊杰 - 不为谁而作的歌.lrc"; // 歌词文件路径
    ...
    @RequestMapping("lrc.action")
    public void getSonglrc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        String path = "F:\\cloudmusicdown\\林俊杰 - 不为谁而作的歌.lrc"; // 歌词文件路径
        TestLRC lrc = new TestLRC();
        List<Map<Long, String>> list = lrc.parse(path);
        request.setAttribute("lrc",list);
        request.getRequestDispatcher("/testlrc/lrctest.jsp").forward(request,response);
    }
}
